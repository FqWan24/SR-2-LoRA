...
class DLoRA(BaseLearner):
    ...
    def incremental_load(self, data_manager, logfilename):
        ...
        self.incremental_eval_draw_shift()
...
def spectral_relation_loss_batch(rel_new, rel_old):
    rel_new_sym = (rel_new + rel_new.transpose(1,2)) / 2
    rel_old_sym = (rel_old + rel_old.transpose(1,2)) / 2

    rel_new_avg = rel_new_sym.mean(dim=0)
    rel_old_avg = rel_old_sym.mean(dim=0).detach()

    evals_new = torch.linalg.eigvalsh(rel_new_avg)
    evals_old = torch.linalg.eigvalsh(rel_old_avg)

    loss = F.smooth_l1_loss(evals_new, evals_old)
    return loss
