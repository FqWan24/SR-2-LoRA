...
class SiNet(nn.Module):
    ...
    def interface(self, image, task_id = None):
        ...
        B, D = feat_last.shape
        cls_list = [layer[:, 0, :] for layer in m_feat]
        cls_all = torch.stack(cls_list, dim=1)
        L = cls_all.shape[1]
        feat_last_exp = feat_last.unsqueeze(1) 
        sim = F.cosine_similarity(cls_all, feat_last_exp, dim=-1)
        # print(sim)
        sim = F.relu(sim-0.4).unsqueeze(-1) 
        feat_align = (sim * cls_all).sum(dim=1)
        feature_enhanced = feat_last + feat_align


        logits = []
        for prompt in self.classifier_pool[:self.numtask]:
            log = prompt(feature_enhanced)
