...
from models.vit_dlora import VisionTransformer, PatchEmbed, Block,resolve_pretrained_cfg, build_model_with_cfg, checkpoint_filter_fn
...
class ViT_lora_co(VisionTransformer):
    ...
    def forward(self, x, task_id, register_blk=-1, get_feat=False, get_cur_feat=False, get_media = False):
...
class SiNet(nn.Module):
    ...
    def forward(self, image, get_feat=False, get_cur_feat=False, fc_only=False):
        ...
        logits = []
        image_features, prompt_loss, new_feature_list = self.image_encoder(image, task_id=self.numtask-1, get_feat=get_feat, get_cur_feat=get_cur_feat, get_media = True)
        image_features = image_features[:,0,:]
        image_features = image_features.view(image_features.size(0),-1)
    
        if self.numtask > 1:
            _, _, old_feature_list = self.image_encoder(image, task_id=self.numtask-2, get_feat=get_feat, get_cur_feat=get_cur_feat, get_media = True)
        else:
            old_feature_list = None
        
        for prompts in [self.classifier_pool[self.numtask-1]]:
            log = prompts(image_features)
            logits.append(log)

        return {
            'logits': torch.cat(logits, dim=1),
            'features': image_features,
            'feature_old_list': old_feature_list, 
            'feature_new_list': new_feature_list, 
            'prompt_loss': prompt_loss
        }

    def interface(self, image, task_id = None):
        image_features, _, m_feat = self.image_encoder(image, task_id=self.numtask-1 if task_id is None else task_id, get_media=True)
        feat_last = image_features[:, 0, :]   # (B, D)
        logits = []
        for prompt in self.classifier_pool[:self.numtask]:
            log = prompt(feat_last)
            # log = log / log.norm(dim=-1, keepdim=True) 
            logits.append(log)
