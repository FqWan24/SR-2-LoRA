...
class Attention_LoRA(nn.Module):
    ...
    def forward(self, x, task, register_hook=False, get_feat=False,get_cur_feat=False, lora_prune_ratio = 0.1):
        if get_feat:
        ...
        #         # ====== 当前 task 的等效权重 W = B @ A ======
        #         Wk = torch.mm(self.lora_B_k[t].weight, self.lora_A_k[t].weight)
        #         Wv = torch.mm(self.lora_B_v[t].weight, self.lora_A_v[t].weight)

        #         # ====== 对当前 task 的 Wk 独立 Top-K 剪枝 ======
        #         if lora_prune_ratio < 1.0:
        #             flat = Wk.view(-1)
        #             k_top = int(len(flat) * lora_prune_ratio)
        #             if k_top > 0:
        #                 thresh = torch.topk(flat.abs(), k_top, largest=True).values.min()
        #                 mask = (Wk.abs() >= thresh).float()
        #                 Wk = Wk * mask

        #         # ====== 对当前 task 的 Wv 独立 Top-K 剪枝 ======
        #         if lora_prune_ratio < 1.0:
        #             flat = Wv.view(-1)
        #             k_top = int(len(flat) * lora_prune_ratio)
        #             if k_top > 0:
        #                 thresh = torch.topk(flat.abs(), k_top, largest=True).values.min()
        #                 mask = (Wv.abs() >= thresh).float()
        #                 Wv = Wv * mask

        #         # ====== 累加每个 task 的稀疏后矩阵 ======
        #         weight_k = weight_k + Wk
        #         weight_v = weight_v + Wv
