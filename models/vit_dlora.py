...
class Attention_LoRA(nn.Module):
    def __init__(
        ...
        # 规范化配置
        self.lora_targets = tuple(t.lower() for t in lora_targets)
        valid_targets = {"q", "k", "v"}
        if not set(self.lora_targets).issubset(valid_targets):
            raise ValueError(f"lora_targets must be subset of {valid_targets}, got {self.lora_targets}")

        # 只为需要的 target 创建 LoRA 参数
    ...
    def _compute_lora_weight(self, A_list, B_list, task):
        """
        累加 0~task 的 LoRA 权重:
        W_delta = sum_t (B_t @ A_t)
        shape: [dim, dim]
        """
        weights = [
            torch.matmul(B_list[t].weight, A_list[t].weight)
    ...
    def forward(self, x, task, register_hook=False, get_feat=False, get_cur_feat=False):
        ...
        if task > -0.5:
            if "q" in self.lora_targets:
                weight_q = self._compute_lora_weight(self.lora_A_q, self.lora_B_q, task)
                y_q = F.linear(x, weight_q)
                q = q + y_q.reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)

            if "k" in self.lora_targets:
                weight_k = self._compute_lora_weight(self.lora_A_k, self.lora_B_k, task)
                y_k = F.linear(x, weight_k)
                k = k + y_k.reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)

            if "v" in self.lora_targets:
                weight_v = self._compute_lora_weight(self.lora_A_v, self.lora_B_v, task)
                y_v = F.linear(x, weight_v)
                v = v + y_v.reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
