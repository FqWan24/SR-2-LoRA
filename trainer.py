...
def _train(args):
    logdir = 'logs/{}/{}_{}_{}/{}/{}/{}'.format(args['dataset'], args['init_cls'], args['increment'], args['net_type'], args['model_name'], args['prefix'], args['optim'])
    args['log_dir'] = logdir
    if not os.path.exists(logdir):
        os.makedirs(logdir)
    logfilename = os.path.join(logdir, '{}_{}'.format(args['seed'], args["disloss"]))
    logging.basicConfig(
        level=logging.INFO,
    ...
    for task in range(data_manager.nb_tasks):
        ...
        logging.info('CNN top1 avg: {}'.format(sum(cnn_curve['top1']) / len(cnn_curve['top1'])))
...
def _test(args):
    ...
    for task in range(data_manager.nb_tasks):
        ...
        model.incremental_load(data_manager, logfilename)
